...
홈 상단의 `DateStrip` 이 월 단위로 날짜를 늘어놓고, 각 날짜에 경기가 있는지 점으로
표시한다(`/api/schedule?month=YYYY-MM`). 경기가 없는 날도 선택은 되며, 누르면
"경기가 없습니다" 상태를 보여준다. 이게 생기면서 헤더의 어제/오늘/내일 버튼과
`2026-07-24` 고정 기본 날짜는 없앴다.

데이터 쪽 제약은 [DATA-SOURCES.md](DATA-SOURCES.md) 참고.
